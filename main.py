...
class Commands(commands.Cog):
    ...
    async def on_message(self, message):
        if self.receiving_answers:
            self.answers.append([message.context, message.author])
        elif self.getting_packname:
            self.packname = message.context

    @commands.command(name='start_quiz')
    async def start_quiz(self, ctx):
        print('здарова теперь квиз команды не работают')
        ...
        for round in range(greet_str[3]):
            ...
            for quest in range(1, self.pack[round][0][2] + 1):
                self.answers = []

                question = self.pack[round][quest][0]
                answers = self.pack[round][1:]
                r_ans = answers[-1]
                shuffle(answers)
